...
## 命名規則（重要，從 week03-3 起遵守）

跳出森林的新世界故事，角色**一律用全新名字**：

- **不要重用森林班底的名字**：米米、阿栗、豆豆、圓圓、跳跳、莉莉、老山羊、老青蛙等都是 W01–W02 森林故事的既定角色，不要把這些名字搬到狐狸／檯燈／棉花雲／小火車等新世界角色上（已上線的 week03 把狐狸叫「阿栗」、week03-2 把女童叫「豆豆」就是反例，造成跨篇混亂）。
- **一篇之內絕不可有兩個同名角色**（week03 原稿出現兩個「豆豆」，已修）。
- 新名字也**避免和最近兩篇的主要角色撞名**。
- 名字要和角色的性別／物種**前後一致**：同一個名字不要這篇是刺蝟、那篇是狐狸。
- 想保留一個熟悉的跨篇錨點時，**只能用「米米」**（小兔），其餘一律新名；但米米非必要，新世界故事可以完全不出現森林角色。
