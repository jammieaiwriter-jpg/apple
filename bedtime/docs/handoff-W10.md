# Codex 交辦單 — W10「選擇」五篇（新模式，一次交）

任務：照配方卡寫 W10「選擇」**五篇一起寫、一起交**。Codex 寫稿、合成音檔並上線；Claude 只做 rubric 審核。

工作目錄 `bedtime/`。

## 先讀
- 五張配方卡：`stories/recipes/week10.md` … `week10-5.md`
...
|----|----|----|----|----|----|
| week10 | 先停下來，想清楚真正需要什麼 | 製作型 | 一隻夜裡才醒的小貓頭鷹 | 書頁之間的微小世界 | 願式 |
| week10-2 | 比較不同選擇可能帶來的結果 | 尋找型 | 一滴小水珠 | 月亮背面的小郵局 | 物件呼應式 |
| week10-3 | 聽聽會受到影響的人的想法 | 來訪型 | 一台舊舊的小收音機 | 池塘邊的荷葉舞台 | 明日式 |
| week10-4 | 做出決定，也接受沒有選到的部分 | 相遇型 | 一隻迷你小火車 | 下雪的屋頂 | 身體放鬆式 |
| week10-5 | 得到新資訊後，可以重新調整選擇 | 等待型 | 一團毛線球 | 雲上的小陽台 | 比喻式 |
...
## 自檢（零 token）
```bash
python3 tools/check-bedtime-story.py week10 week10-2 week10-3 week10-4 week10-5
```
全部通過、五篇交回給 Claude 審。

## Claude 審核通過後（這批改由 Codex 收尾）
1. 把這五篇的 `review_status` 改 `adult_verified`（依 Claude 的審核結論；待修的先改）。
2. `stories/catalog.json` 的 W10 episode 補上這五篇（title/focus/status/available），episode 設 available:true、title 用主題「選擇」。
3. 合成音檔（兩套）：
   ```bash
   python3 tools/generate-bedtime-audio.py week10 week10-2 week10-3 week10-4 week10-5
   python3 tools/generate-bedtime-audio.py week10 week10-2 week10-3 week10-4 week10-5 --name "光哥、阿築" --suffix gz
   ```
   確認每篇時長 8–10 分；再跑 `python3 tools/check-bedtime-story.py week10 week10-2 week10-3 week10-4 week10-5` 應全綠。
