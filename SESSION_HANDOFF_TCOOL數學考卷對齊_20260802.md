...
- 發布判定：**NO-GO**
- 2026-08-02 產品決策：**凍結現有數學 HTML 內容擴充**；目前內容過於單調且深度不足，必須先完成試卷逐題對齊與題型矩陣，再生成題庫與 HTML。
...
HTML 是最後的呈現層，不是內容來源。本階段不得再做頁面美化、擴題或即時 AI 生題。重啟 HTML 的最低條件是：

1. 首發範圍的題目已逐題對齊，且答案、圖像與課綱證據通過 release gate。
2. 題型矩陣已明確描述「單元 × 觀念 × 題型 × 表徵 × 問法 × 陷阱 × 難度 × 證據題」。
3. 矩陣能支撐多元變化，不只是更換數字；至少要改變情境、表徵、提問方向或常見陷阱。
4. 預生成題、答案、解題與 retry variants 已完成獨立驗算。
...
6. **題型矩陣**：單元 × 觀念 × 題型 × 表徵 × 問法 × 陷阱 × 難度 × evidenceQuestionIds；並用頻率、風險、證據強度及孩子答題狀況決定配題權重。
7. **12 題 MVP**：先做單一範圍的 12 題短回合及每題兩個離線 retry variants，全部獨立驗算。
8. **隨堂練習／段考測試 UI**：通過上述 gate 後才重做 HTML；不得即時 AI 生題，只載入 verified 題庫。
